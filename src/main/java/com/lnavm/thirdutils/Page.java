package com.lnavm.thirdutils;

import com.fasterxml.jackson.annotation.JsonIgnore;
import com.lnavm.Config.Constant;
import com.lnavm.Config.Global;
...
public class Page<T> implements Serializable{
    ...
    {
        try {
            pageSize = Integer.parseInt(Global.getConfig(Constant.INIT_PAGE_SIZE));
        }
        catch (Exception e) {
            pageSize = 10; // 默认的页面大小
        }
	...
	public void initialize(){
		...
		if((count % (this.pageSize < 1 ? 20 : this.pageSize)) != 0 || this.last == 0) {
			this.last += 1;
		}
		...
		pageList.clear();
		int start = pageNo-2 >= first ? pageNo-2 : first;
		int stop = (pageNo+2) <= last ? pageNo+2 : last;
